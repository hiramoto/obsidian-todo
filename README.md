...
- フロントマターに `task_id` を持つタスクファイルが対象
...
タスクフォルダ内の `.md` ファイルが作業開始時の選択対象になります。フロントマターに `status: in-progress` があるタスクは一覧の先頭に表示されます。

作業履歴の自動表示を有効にするには、フロントマターに `task_id` を設定してください。
